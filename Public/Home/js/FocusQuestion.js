...
$(function(){
	$(document).on('click','.focus-question-btn',function(){
		//获取当前问题id
		var qid=$(this).closest('.index_my_left_category').find('.hide_question_id').val();
		var mythis = $(this);
		$.ajax({
				type:'post',
				dataType:'json',
				...
				success:function(data){
					...
					if(data.status==1){
						...
						if(data.content.status==1){
							...
							//此时用户已关注问题，渲染为取消关注
							mythis.html("<img src='"+HOME_IMAGES+"/nofocus.png'/>取消关注");
						   
						}else if(data.content.status==0){
							//此时用户已取消关注问题，渲染为关注问题
							mythis.html("<img src='"+HOME_IMAGES+"/add.png'/>关注问题");
